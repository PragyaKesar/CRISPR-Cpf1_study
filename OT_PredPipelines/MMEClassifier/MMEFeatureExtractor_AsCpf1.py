...
class AsCpf1_MMEFeatureExtractor:
    def __init__(self):
        pass
        ...
        for seqs, seqs1 in zip(h1,h2):
            ...
            for j, k in zip(PAM, PAM1):
                ...
                if j and k == "A" and i==2:
                    dna_encode = np.append(dna_encode, np.array([9999]).T)
                    dna_encodei = np.append(dna_encodei, np.array([9999]).T)
    ...
    def ModelPred(self):
        ...
        Prediction = joblib.load('MMEClassifier_AsCpf1.pkl')
